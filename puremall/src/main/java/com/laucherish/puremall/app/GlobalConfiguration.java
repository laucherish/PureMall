package com.laucherish.puremall.app;
 ...
 * @see
 * <a href="https://github.com/JessYanCoding/ArmsComponent/wiki">MVPArms 官方组件化方案 ArmsComponent, 进阶指南!</a>
...
//    public static String sDomain = Api.APP_DOMAIN;
    ...
    public void applyOptions(Context context, GlobalConfigModule.Builder builder) {
        ...
        builder.baseurl(Api.APP_DOMAIN)
                ...
                //想支持多 BaseUrl, 以及运行时动态切换任意一个 BaseUrl, 请使用 https://github
                // .com/JessYanCoding/RetrofitUrlManager
                //如果 BaseUrl 在 App 启动时不能确定, 需要请求服务器接口动态获取, 请使用以下代码
                //以下方式是 Arms 框架自带的切换 BaseUrl 的方式, 在整个 App 生命周期内只能切换一次, 若需要无限次的切换 BaseUrl,
                // 以及各种复杂的应用场景还是需要使用 RetrofitUrlManager 框架
                //以下代码只是配置, 还要使用 Okhttp (AppComponent 中提供) 请求服务器获取到正确的 BaseUrl 后赋值给
                // GlobalConfiguration.sDomain
...
//                                                  String headers, MediaType contentType, String
//                                                  bodyString,
//                                                  List<String> segments, String message, String
//                                                  responseUrl) {
...
//                .executorService(Executors.newCachedThreadPool())
                ...
                //这里提供一个全局处理 Http 请求和响应结果的处理类, 可以比客户端提前一步拿到服务器返回的结果, 可以做一些操作, 比如 Token 超时后, 重新获取
                // Token
                ...
                .gsonConfiguration((context1, gsonBuilder) -> {//这里可以自己自定义配置 Gson 的参数
                    gsonBuilder
                            ...
                            .enableComplexMapKeySerialization();//支持将序列化 key 为 Object 的 Map,
                    // 默认只能序列化 key 为 String 的 Map
                })
                .retrofitConfiguration((context1, retrofitBuilder) -> {//这里可以自己自定义配置 Retrofit
...
//                    okhttpBuilder.sslSocketFactory(); //支持 Https, 详情请百度
                    ...
                    //使用一行代码监听 Retrofit／Okhttp 上传下载进度监听, 以及 Glide 加载进度监听, 详细使用方法请查看
                    // https://github.com/JessYanCoding/ProgressManager
//                    ProgressManager.getInstance().with(okhttpBuilder);
                    //让 Retrofit 同时支持多个 BaseUrl 以及动态改变 BaseUrl, 详细使用方法请查看 https://github
                    // .com/JessYanCoding/RetrofitUrlManager
